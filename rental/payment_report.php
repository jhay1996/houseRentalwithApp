				...
				<div class="col-md-12">
					<form id="filter-report">
						<div class="row form-group">
							...
							<button style="background-color: #ed6801; border-color: #ed6801; color: white;" class="btn btn-sm btn-block btn-primary col-md-2 ml-1">Filter</button>
						</div>
					</form>
					<hr>
						<div class="row">
							<div class="col-md-12 mb-2">
							<button style="background-color: #ed6801; border-color: #ed6801; color: white;" class="btn btn-sm btn-block btn-success col-md-2 ml-1 float-right" type="button" id="print"><i class="fa fa-print"></i> Print</button>
							</div>
						</div>
					<div id="report">
						<div class="on-print">
						...
						<div class="row">
							<table class="table table-bordered">
								...
								<tbody>
									...
									$i = 1;
									$tamount = 0;
									$payments  = $conn->query("SELECT p.*,fullname as name,h.house_no FROM payments p inner join tenants t on t.id = p.tenant_id inner join houses h on h.id = t.house_id where date_format(p.date_created,'%Y-%m') = '$month_of' order by unix_timestamp(date_created)  asc");
